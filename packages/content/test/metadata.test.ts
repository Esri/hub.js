...
import { getContentMetadata, parseMetadataXml } from "../src/metadata";
import * as datasetWithMetadata from "./mocks/datasets/feature-layer-with-metadata.json";

describe("metadata", () => {
  describe("getContentMetadata", () => {
    it("should call getItemMetadata and parse", done => {
      const id = "foo";
      const xml = "<metadata><about></about></metadata>";
      const parsed = { cool: true };
      const getItemMetadataStub = spyOn(
        arcgisRestPortal,
        "getItemMetadata"
      ).and.returnValue(Promise.resolve(xml));
      const parseStub = spyOn(fastXmlParser, "parse").and.returnValue(parsed);
      getContentMetadata("foo").then(metadata => {
        expect(metadata).toEqual(parsed);
        expect(getItemMetadataStub.calls.argsFor(0)[0]).toEqual(id);
        expect(parseStub.calls.argsFor(0)[0]).toEqual(xml);
        done();
      });
    });
  });
  describe("parseMetadataXml", () => {
    ...
    it("should parse dataset metadata XML", () => {
      ...
      // the duke used an older version of fast-xml-parser that:
      ...
      expected.metadata.Esri.DataProperties.coordRef.peXml =
        result.metadata.Esri.DataProperties.coordRef.peXml;
