...
describe("requestDownloadMetadata", () => {
  ...
  it("handle portal download, single layer CSV", async done => {
    ...
    try {
      ...
      fetchMock.mock(
        "http://portal.com/sharing/rest/search?f=json&q=type%3A%22CSV%22%20AND%20typekeywords%3A%22export%3Aabcdef0123456789abcdef0123456789%2CspatialRefId%3A2227%22&num=1&sortField=modified&sortOrder=DESC&token=123",
        {
          status: 200,
          body: {
            results: []
      ...
      expect(result).toEqual({
        downloadId:
          "abcdef0123456789abcdef0123456789:CSV:2227:undefined:undefined",
  ...
  it("handle portal download, mulit-layer CSV", async done => {
    const authentication = new UserSession({
      username: "portal-user",
      portal: `http://portal.com/sharing/rest`,
      token: "123"
