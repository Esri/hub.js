...
describe("shareWithGroup", function () {
  ...
  let shareWithGroupRevertablePromiseResults: Promise<hubCommon.IRevertableTaskResult>[];
  ...
  it("should resolve the results from processRevertableTasks", async function () {
    ...
    expect(shareWithGroupRevertableSpy.calls.argsFor(2)).toEqual([fieldworkerModel, ViewGroup, requestOptions]);
    expect(shareWithGroupRevertableSpy.calls.argsFor(3)).toEqual([stakeholderModel, ViewGroup, requestOptions]);
    expect(processRevertableTasksSpy.calls.count()).toBe(1);
    expect(processRevertableTasksSpy.calls.argsFor(0)).toEqual([shareWithGroupRevertablePromiseResults]);
  ...
  it("should reject if processRevertableTasks rejects", async function (done) {
    ...
    try {
      ...
      expect(processRevertableTasksSpy.calls.count()).toBe(1);
      expect(processRevertableTasksSpy.calls.argsFor(0)).toEqual([shareWithGroupRevertablePromiseResults]);
      expect(e).toEqual(new Error(`Failed to share survey ${formModel.item.id} items to group ${ViewGroup.id}`));
      done();
    }
  });
});
