    ...
    IWithCapabilityBehavior
{
  ...
  //#region IWithResourceBehavior
  async clearResource(resourceName: string): Promise<void> {
    try {
      // remove the resource
      const response = await removeItemResource({
        id: this.entity.id,
      ...
      if (response && !response.success) {
        throw new HubError(
          "Clear Item Resource",
          `Unknown error clearing resource. ${resourceName}`
        );
      }
      // TODO: report to class that resource was removed
      // and that it needs to clear it out...
    } catch (err) {
      if (err instanceof Error) {
        throw new HubError("Clear Item Resource", err.message, err);
      } else {
        throw new HubError(
          "Clear Item Resource",
          `Unknown error clearing resource. ${resourceName}`
  ...
  async addResource(
    ...
    resource: any,
    prefix: string = "",
    clearOldResource: boolean = false,
    stringOrJson?: "string" | "json"
  ): Promise<void> {
    try {
      if (clearOldResource) {
        this.clearResource(resourceName);
      }
      ...
      if (err instanceof Error) {
        ...
        throw new HubError(
          "Add Item Resource",
          `Unknown error adding resource. ${resourceName}`
        );
      }
    }
  }
