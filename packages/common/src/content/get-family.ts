...
export function getFamily(itemOrType: IItem | string) {
  const item = itemOrType as IItem;
  // TODO: should we call normalizeItemType on this?
  const type = item.type || (itemOrType as string);
  ...
  switch (type.toLowerCase()) {
    case "image service":
      family = typeKeywords?.includes("Tiled Imagery") ? "map" : "dataset";
      break;
