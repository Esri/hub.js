...
fdescribe("index", () => {
  ...
  const NODE_EXPORTS = [...EXPORTED_TYPES, ...EXPORTED_ARTIFACTS];
  const EXPORTED = Object.keys(hubCommon);
  const EXPECTED =
    typeof window === "undefined" ? NODE_EXPORTS : BROWSER_EXPORTS;

  it("should export the expected number of artifacts", () => {
    expect(EXPORTED.length).toEqual(EXPECTED.length);
  });

  it("should not add any unexpected exports", () => {
    const added = EXPORTED.filter(
      (exportedMember) => !EXPECTED.includes(exportedMember)
    );
    // console.log("====");
  ...
  it("should not unexpectedly remove any exports", () => {
    const removed = EXPECTED.filter(
      (expectedMember) => !EXPORTED.includes(expectedMember)
    );
    // console.log("====");
