...
import { CatalogSetupType } from "../../src/search/types";
...
describe("initCatalogOnEntityCreate", () => {
  ...
  it("creates a new group and uses its id when type is newGroup", async () => {
    const getWellKnownGroupSpy = spyOn(
      getWellKnownGroupModule,
      "getWellKnownGroup"
    ).and.returnValue({ foo: "bar" });
    const createHubGroupSpy = spyOn(
      createHubGroupModule,
      "createHubGroup"
    ).and.returnValue(Promise.resolve({ id: "newGroupId" }));
