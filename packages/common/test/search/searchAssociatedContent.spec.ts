...
describe("searchAssociatedContent", () => {
  let mockGetOgcCollectionUrl: ReturnType<typeof vi.spyOn>;
  let mockSearchOgcItems: ReturnType<typeof vi.spyOn>;

  beforeEach(() => {
    mockGetOgcCollectionUrl = vi
      .spyOn(getOgcCollectionUrlModule, "getOgcCollectionUrl")
      .mockReturnValue("https://example.com/path/to/ogc/collections/all");

    mockSearchOgcItems = vi
      .spyOn(searchOgcItemsModule, "searchOgcItems")
  ...
  it("throws an error if entity is not item-based", async () => {
    ...
    try {
      ...
      throw new Error("Expected HubError to be thrown");
    } catch (error) {
