import { searchCatalogs } from "../../src/search/searchCatalogs";
import { IHubCatalog } from "../../src/search/types/IHubCatalog";
import { IArcGISContext } from "../../src/types/IArcGISContext";
import { describe, it, expect, afterEach, vi } from "vitest";
import { Catalog } from "../../src/search/Catalog";

describe("searchCatalogs:", () => {
  afterEach(() => vi.restoreAllMocks());

  it("calls searchCollections on all catalogs", async () => {
    const spy = vi
      .spyOn(Catalog.prototype as any, "searchCollections")
      .mockImplementation(() => {
        return Promise.resolve({
          col1: {
            hasNext: false,
            results: [],
            total: 10,
          },
          col2: {
            hasNext: false,
            results: [],
            total: 20,
          },
        });
      });
    const catalogs: IHubCatalog[] = [
      { title: "catalog1", schemaVersion: 1 } as unknown as IHubCatalog,
      { title: "catalog2", schemaVersion: 1 } as unknown as IHubCatalog,
    ];
    const query = "water";
    const options = {};
    const ctx = {} as unknown as IArcGISContext;
    const result = await searchCatalogs(catalogs, query, options, ctx);
    expect(spy).toHaveBeenCalledTimes(2);
    expect(result.length).toBe(2);
    expect(result[0].catalogTitle).toBe("catalog1");
    expect(result[1].catalogTitle).toBe("catalog2");
  });

  it("uses scope if no collection", async () => {
    const searchCollectionsSpy = vi
      .spyOn(Catalog.prototype as any, "searchCollections")
      .mockResolvedValue({});
    const searchScopesSpy = vi
      .spyOn(Catalog.prototype as any, "searchScopes")
      .mockResolvedValue({
        items: {
          hasNext: false,
          results: [],
          total: 10,
        },
      });
    const catalogs: IHubCatalog[] = [
      {
        title: "catalog1",
        schemaVersion: 1,
        scopes: {
          item: {
            targetEntity: "item",
            filters: [{ predicates: [{ group: "00c" }] }],
          },
        },
        collections: [
          {
            key: "myGroups",
            label: "My Groups",
            targetEntity: "group",
            scope: {
              targetEntity: "group",
              filters: [{ predicates: [{ owner: "skywalker" }] }],
            },
          },
        ],
      },
    ];
    const query = "water";
    const options = {};
    const ctx = {} as unknown as IArcGISContext;
    await searchCatalogs(catalogs, query, options, ctx);
    expect(searchCollectionsSpy).toHaveBeenCalledTimes(1);
    expect(searchScopesSpy).toHaveBeenCalledTimes(1);
  });
});
