...
describe("explainPredicate:", () => {
  ...
  it("matchOptionsPredicate", async () => {
    const fn = vi
      .spyOn(explainHelpersModule, "explainMatchOptionPredicate")
      .mockResolvedValue({ included: true });
  ...
  it("DatePredicate", async () => {
    const fn = vi
      .spyOn(explainHelpersModule, "explainDatePredicate")
      .mockResolvedValue({ included: true });
    const predicate: IPredicate = {
      created: {
        type: "relative-date",
  ...
  it("PropPredicate", async () => {
    const fn = vi
      .spyOn(explainHelpersModule, "explainPropPredicate")
      .mockResolvedValue({ included: true });
    const predicate: IPredicate = {
      name: "yoda",
    };
