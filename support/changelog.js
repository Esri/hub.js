...
const repo = "https://github.com/Esri/hub.js";
...
function getCommitData(from, to) {
  return new Promise((resolve, reject) => {
    ...
    exec(cmd, (err, stdout, stderr) => {
      ...
      /*
       NOTE: you will get paring errors on the next step if your commit messages have special chars like "
      ...
      const commits = JSON.parse("["+stdout.slice(0, -1).replace(/Revert "/, 'Revert ').replace(/metadata if it exists"/, 'metadata if it exists') +"]").reverse();
