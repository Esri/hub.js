...
describe("getUserCreatableTeams", () => {
  it("respects product and privs for 9.1", () => {
    ...
    expect(getUserCreatableTeams(user, "premium", "In House").length).toBe(
      6,
      "Premium, all privs, should be 6 creatable team types"
    );

    expect(getUserCreatableTeams(user, "basic", "In House").length).toBe(
      3,
      "Basic, all privs, should be 3 creatable team types"
    );

    expect(getUserCreatableTeams(user, "portal", "In House").length).toBe(
      4,
      "Portal, all privs, should be 4 creatable team types"
    );
  ...
  it("properly updates membershipAccess... when warranted", () => {
    ...
    const teams = getUserCreatableTeams(user, "premium", "In House");
